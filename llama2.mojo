...
fn convert_to_str(s: PointerString) -> String:
    # convert raw byte like <0x0A>
    if (s[1].to_int() == ord("0")) and (s[2].to_int() == ord("x")):
        let d1: Int = s[3].to_int()
        let d2: Int = s[4].to_int()
        return chr(str2num(d1) * 16 + str2num(d2))
    # convert all chars till null character
    var p: Int = 0
    var final_str: String = ""
    while s[p].to_int() != 0:
        let next_str = chr(s[p].to_int())
        p += 1
        final_str += next_str
    return final_str
...
fn run(prompt: String) raises -> String:
    ...
    while pos < steps:
        ...
        let next_str = convert_to_str(token_str)
        final_str += next_str
